...
const Swap = () => {
  ...
  const calculateValue = () => {
    ...
    axios
      .get("http://localhost:8080/api/tools/convert", {
      ...
      .then((response) => {
        ...
        setValue({
          loading: false,
          value: response.data.data.price.toFixed(10),
        });
      })
  ...
  const selectCoinFunction = (who) => {
    ...
    return (
      <SelectedCoinButton onClick={(e) => handleSelectButton(e, who)}>
        <img
          src={innerCoin.marketData.image}
          alt={innerCoin.id}
  ...
  const swapNowHandler = () => {
    ...
    axios
      .post("http://localhost:8080/api/trade", null, {
        headers: { "Access-Control-Allow-Origin": "*" },
        auth: {
          username: sessionStorage.getItem("username"),
          password: sessionStorage.getItem("password"),
  ...
  if (status === true) return <Navigate to="/portfolio" />;
            ...
            <SwapContainer>
              <TitleContainer>
                ...
                <InnerSwapContainer>
                  ...
                  <SwapButton onClick={swapNowHandler}>Swap Now</SwapButton>
                </InnerSwapContainer>
              </TitleContainer>
            </SwapContainer>
          </InnerContainer>
